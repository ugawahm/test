...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.w = nn.Parameter(torch.Tensor([0.3, 0.7]), requires_grad=False)
        ...
        self.cbam = CBAMLayer(256)          # 输入channel应与实际输入深度匹配
    ...
    def forward(self, x):
        ...
        # # ————————————2—————————————
        x = self.vcr.forward(x)                  # [batch, 2048, 7, 7]
        # x = torch.cat((x, cbam), 1)            # 1) concat融合，需改参数，见360，361行
        # x = torch.add(cbam, x)                 # 2)特征相加
        x = cbam * self.w[0] + x * self.w[1]     # 3)特征加权 cbam 0.3 + vcr 0.7
